...
@LastEditors: Seven
@LastEditTime: 2020-05-23 08:17:59
...
def worker():
    ...
    while True:
        ...
        print(
            "GPU RAM Free:{0:.0f}MB, Used:{1:.0f}MB, Util:{2:3.0f}%, Total:{3:.0f}MB, Load:{4:.0f}%".format(
                gpu.memoryFree, gpu.memoryUsed, gpu.memoryUtil * 100, gpu.memoryTotal, gpu.load
            )
        )
...
def monitor():
    ...
    """
    thread_monitor = threading.Thread(target=worker, name="Monitor")
